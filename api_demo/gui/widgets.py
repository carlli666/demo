...
class RequestPanel(ttk.Frame):
    ...
    公开方法：
      ...
      - clear(): 清空所有输入
      - apply_theme(theme_manager): 切换主题时调用
    """
    ...
    def __init__(
        ...
        if self.theme_manager is not None:
            ...
            self._color_method(self.var_method.get())
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(top, text="方法:", font=font_ui).grid(row=0, column=0, padx=(0, PAD_SM))
        ...
        self.cmb_method.grid(row=0, column=1, sticky="w", padx=(0, PAD_MD))
        # 虚拟事件 <<MethodChanged>> 在 _color_method 后触发
        self.cmb_method.bind("<<ComboboxSelected>>", self._on_method_change)

        # 方法色标签（点睛之笔：根据方法变色）
        self.lbl_method_color = tk.Label(
            top, text="●", font=("Segoe UI", 14), padx=0, pady=0
        )
        self.lbl_method_color.grid(row=0, column=2, sticky="w", padx=(0, PAD_SM))

        ttk.Label(top, text="路径:", font=font_ui).grid(row=0, column=3, padx=(PAD_MD, PAD_SM))
        self.entry_path = ttk.Entry(top, textvariable=self.var_path, font=font_ui)
        self.entry_path.grid(row=0, column=4, sticky="ew")

        self.var_path.set("/users/123")

        # === 第 1 行：分隔条 ===
        ttk.Separator(self, orient="horizontal").grid(row=1, column=0, sticky="ew", pady=PAD_SM)
        ...
        self._build_body_tab(self.notebook, font_ui, mono)

        # === 第 3 行：发送按钮 + 状态 ===
        bottom = ttk.Frame(self)
        bottom.grid(row=3, column=0, sticky="ew", pady=(PAD_SM, 0))
        ...
        self.status_label = ttk.Label(
            bottom, text="● 就绪", font=font_ui, style="Muted.TLabel"
        )
        self.status_label.grid(row=0, column=1, sticky="w", padx=(PAD_MD, 0))

        # 注册到主题管理器
        if self.theme_manager is not None:
            for label in [self.lbl_method_color, self.status_label]:
                if label.winfo_class() == "Label":
                    self.theme_manager.register(label)

    def _build_params_tab(self, parent: tk.Misc, font: tuple) -> None:
        frame = ttk.Frame(parent, padding=PAD_MD)
        parent.add(frame, text=f"Params ({PARAMS_ROWS} 行)")

        header = ttk.Frame(frame)
        header.pack(fill="x")
        ttk.Label(header, text="键", width=20, font=font).pack(side="left", padx=(0, PAD_SM))
        ttk.Label(header, text="值", font=font).pack(side="left")

        rows_box = ttk.Frame(frame)
        rows_box.pack(fill="both", expand=True, pady=(PAD_SM, 0))

        for _ in range(PARAMS_ROWS):
            row = ttk.Frame(rows_box)
            row.pack(fill="x", pady=2)
            k = ttk.Entry(row, width=22, font=font)
            v = ttk.Entry(row, font=font)
            k.pack(side="left", padx=(0, PAD_SM))
    ...
    def _build_body_tab(self, parent: tk.Misc, font: tuple, mono: tuple) -> None:
        frame = ttk.Frame(parent, padding=PAD_MD)
        ...
        if self.theme_manager is not None:
            self.theme_manager.register(self.body_text)
        ...
        btn_bar.pack(fill="x", pady=(PAD_SM, 0))
    ...
    def _on_method_change(self, _event: Optional[tk.Event] = None) -> None:
        """切换方法时更新方法色 + Body 页启用态。"""
        method = self.var_method.get()
        self._color_method(method)
        ...
        if method in METHODS_WITH_BODY:
            ...
            self.status_label.configure(text=f"● {method} 不需要 Body")

    def _color_method(self, method: str) -> None:
        """根据方法设置左侧色点颜色。"""
        if self.theme_manager is None:
            return
        color = self.theme_manager.method_color(method)
        try:
            self.lbl_method_color.configure(foreground=color)
        except tk.TclError:
            pass

    def _format_body(self) -> None:
        """把 Body 文本框内容格式化为标准 JSON。"""
        if self.body_text is None:
            return
    ...
    def fill_example(self) -> None:
        ...
        self.status_label.configure(text="● 已填入示例")
...
class ResponsePanel(ttk.Frame):
    ...
    def __init__(self, master: tk.Misc, theme_manager=None):
        super().__init__(master)
        self.theme_manager = theme_manager
        ...
        # 第 0 行：状态 + 耗时
        self.var_status = tk.StringVar(value="等待请求")
        ...
        self.lbl_status = ttk.Label(top, textvariable=self.var_status, font=("Segoe UI", 11, "bold"))
        self.lbl_status.grid(row=0, column=0, sticky="w")

        self.lbl_elapsed = ttk.Label(
            top, textvariable=self.var_elapsed, font=font_ui, style="Muted.TLabel"
        )
        self.lbl_elapsed.grid(row=0, column=1, sticky="w", padx=(PAD_MD, 0))
        ...
        if self.theme_manager is not None:
            self.theme_manager.register(self.text)
        ...
        if self.theme_manager is not None:
            ...
            self.text.tag_configure("success", foreground=palette["success"])
            self.text.tag_configure("meta", foreground=palette["text_muted"])
        else:
            self.text.tag_configure("error", foreground="#ef4444")
            self.text.tag_configure("success", foreground="#10b981")
            self.text.tag_configure("meta", foreground="#64748b")

        # 第 2 行：复制按钮
        btn_bar = ttk.Frame(self)
        btn_bar.grid(row=2, column=0, sticky="ew", pady=(PAD_SM, 0))
    ...
    def set_status(self, status: Optional[int], elapsed_ms: Optional[float] = None) -> None:
        """更新顶部状态行。"""
        if status is None:
            self.var_status.set("等待请求")
            self._color_status_label("#94a3b8")
        else:
            self.var_status.set(f"HTTP {status}")
            if 200 <= status < 300:
                self._color_status_label("#10b981" if self._is_light() else "#34d399")
            elif status >= 400:
                self._color_status_label("#ef4444" if self._is_light() else "#f87171")
            else:
                self._color_status_label("#64748b")

        if elapsed_ms is None:
            self.var_elapsed.set("")
        else:
            self.var_elapsed.set(f"耗时 {elapsed_ms:.0f} ms")

    def _color_status_label(self, color: str) -> None:
        try:
    ...
    def _is_light(self) -> bool:
        ...
        return self.theme_manager.current == Theme.LIGHT
