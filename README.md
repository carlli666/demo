...
# 2. 打包
pyinstaller --onefile --windowed --name "intelligent assistant" --icon intelligent_assistant.ico --add-data "intelligent_assistant.ico;." run_gui.py
```
...
- 图标来源：`intelligent_assistant.ico`（由 `make_icon.py` 生成，无外部依赖）
- 可以把它拖到桌面、钉到任务栏、拷给同事
