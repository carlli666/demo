...
### ✨ 新增
- **在线升级功能**：通过 GitHub Releases 自动检查 + 下载 + 安装新版
  - 启动后台静默检查（默认 7 天一次，不打扰用户）
  - 设置对话框加「🔄 检查更新」按钮手动触发
  - 发现新版本弹友好对话框（版本号 + 更新说明 + 立即更新/稍后/跳过）
  - 下载带进度条 + 速度显示
  - 校验 SHA256 完整性
  - 用批处理脚本替换 exe（Windows 文件锁问题的标准方案）
  - 新增 27 个单元测试（版本解析、API mock、SHA256 等）
